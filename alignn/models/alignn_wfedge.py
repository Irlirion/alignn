...
class ALIGNNWF(nn.Module):
    ...
    def __init__(self, config: ALIGNNConfig = ALIGNNConfig(name="alignn_wf_edge")):
        ...
        self.wfrbf_embedding = nn.Sequential(
            MLPLayer(128, config.embedding_features),
            MLPLayer(config.embedding_features, config.hidden_features),
        )
        ...
        self.edge_lin = nn.Linear(config.hidden_features * 2, config.hidden_features)
    ...
    def forward(
        ...
        y = torch.cat([self.edge_embedding(bondlength), self.wfrbf_embedding(g.edata.pop("add_wfrbf"))], dim=-1)
        y = self.edge_lin(y)
